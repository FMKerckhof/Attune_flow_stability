...
attuneYGbeads.trf.beads.QC_LR <- flowAIcallR(attuneYGbeads.trf.beads,
                                          ...
                                          timesplit = 0.5)
### Attune with prf flow AI calls YG -------------------------------------------
...
eventsAttuneYGQC_UHR <- fsApply(attuneYGbeads.trf.beads.QC_UHR,
                                function(x)nrow(x))
eventsAttuneYGQC_HR <- fsApply(attuneYGbeads.trf.beads.QC_HR,function(x)nrow(x))
eventsAttuneYGQC_LR <- fsApply(attuneYGbeads.trf.beads.QC_LR,function(x)nrow(x))
...
AttuneYGQCfilterinfo.meta   <- dplyr::inner_join(AttuneYGQCfilterinfo,mddf,
                                                 by="Filename")
FACSVerseYGQCfilterinfo.meta<- dplyr::inner_join(FACSVerseYGQCfilterinfo,mddf,
                                                 by="Filename")
...
### Plotting -------------------------------------------------------------------
plot.global <- YGfilterinfo.meta.all %>% ggplot(aes(x=as.factor(Dilution),
                                                    y=percentRemoved,
                                                    fill=Cytometer,
                                                    color=Cytometer))
